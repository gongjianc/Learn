...
ip_glue(struct ipq * qp)
{
  ...
  /*******************恢复skb数据包更新，保证碎片还原后数据能够完整返回给上层调用****************************/
  // skb->ip_hdr = iph;
  //(struct ip *)skb->ip_hdr = iph;
...
ip_defrag_stub(struct ip *iph, struct ip **defrag)
{
  ...
  if (((flags & IP_MF) == 0) && (offset == 0)) {
    ip_defrag(iph, 0);
//    printf("There is no ip_frag packet arrived!!\n"); 
    return IPF_NOTF;
  }
  //printf("There is something for ip_frag packet!!\n"); 
  ...
  if ((*defrag = (struct ip *)ip_defrag((struct ip *) (skb->data), skb))){
    //printf("There is a new ip_frag packet arrived!!\n"); 
    return IPF_NEW;}
  
  //printf("There is nothing for ip_frag packet arrived!!\n"); 
  return IPF_ISF;
}
