...
static void
add_tcp_closing_timeout(struct tcp_stream * a_tcp, struct pcap_pkthdr *hdr)
{
  ...
  newto->a_tcp = a_tcp;
  newto->timeout.tv_sec = hdr->ts.tv_sec + 10;
  newto->prev = 0;
             ...
             struct pcap_pkthdr *hdr)
{
  ...
  if (fin) {
    ...
    if (rcv->state == TCP_CLOSING)
      add_tcp_closing_timeout(a_tcp,hdr);
  }
}
...
tcp_queue(struct tcp_stream * a_tcp, struct tcphdr * this_tcphdr,
	  ...
	  char *data, int datalen, int skblen, struct pcap_pkthdr *hdr
	  )
{
  ...
  if (!after(this_seq, EXP_SEQ)) {
    if (after(this_seq + datalen + (this_tcphdr->th_flags & TH_FIN), EXP_SEQ)) {
      ...
      add_from_skb(a_tcp, rcv, snd, (u_char *)data, datalen, this_seq,
		   ...
		   ntohs(this_tcphdr->th_urp) + this_seq - 1, hdr);
      /*
	...
	if (after(pakiet->seq + pakiet->len + pakiet->fin, EXP_SEQ)) {
	  add_from_skb(a_tcp, rcv, snd, pakiet->data,
		       ...
		       pakiet->urg_ptr + pakiet->seq - 1, hdr);
        }
...
//  printf("hash_index is %d!!!!!!!!!\n",hash_index);
  a_tcp = tcp_stream_table[hash_index];
//  printf("a_tcp->addr.source is %d!!!!!!!!!\n",&a_tcp->addr.source);
//  printf("a_tcp->addr.dest is %d!!!!!!!!!\n",&a_tcp->addr.dest);
//  printf("a_tcp->addr.saddr is %d!!!!!!!!!\n",&a_tcp->addr.saddr);
//  printf("a_tcp->addr.daddr is %d!!!!!!!!!\n",&a_tcp->addr.daddr);
...
void
process_tcp(u_char * data, int skblen, struct pcap_pkthdr *hdr, struct tuple5 *Five_attr)
{
...
//ECN
  ...
  if (from_client) {
    if((Five_attr->protocol != 0)&&(!(memcmp(&a_tcp->addr, &Five_attr->four_attr, sizeof(struct tuple4))))){
       a_tcp->addr5.protocol = Five_attr->protocol;
       //a_tcp->addr5.flow_flag = Five_attr->flow_flag;
      //a_tcp->addr5.flow = Five_attr->flow;
       //printf("Tcp flow_flag is %d\n",Five_attr->flow_flag);
    }
    snd = &a_tcp->client;
    rcv = &a_tcp->server;
  }
    ...
    reversed.source = Five_attr->four_attr.dest;
    reversed.dest = Five_attr->four_attr.source;
    reversed.daddr = Five_attr->four_attr.saddr;
    reversed.saddr = Five_attr->four_attr.daddr;    
    if((Five_attr->protocol != 0)&&(!(memcmp(&a_tcp->addr, &reversed, sizeof(struct tuple4))))){
       a_tcp->addr5.protocol = Five_attr->protocol;
      // a_tcp->addr5.flow_flag = Five_attr->flow_flag;
      // a_tcp->addr5.flow = Five_attr->flow;
       //printf("Tcp flow_flag is %d\n",Five_attr->flow_flag);
