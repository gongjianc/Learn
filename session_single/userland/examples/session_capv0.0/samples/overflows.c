...
void detect_imap(struct tcp_stream *a_tcp)
{
	char numbuf[30];
	...
	if (atoi(numbuf) > 1024) {
		// notify admin
		syslog(nids_params.syslog_level, "Imapd exploit attempt, connection %s\n", adres(a_tcp->addr));
		// kill the connection
		nids_killtcp(a_tcp);
	}
	nids_discard(a_tcp, datalen - PATLEN);
...
int add_to_path(char *path, char *elem, int len)
{
	int plen;
	char *ptr;
	if (len > 768)
	...
	if (len == 2 && elem[0] == '.' && elem[1] == '.') {
		ptr = rindex(path, '/');	//此函数用来找出path字符串中最后一个出现`/`的位置，并返回地址位置。
		if (ptr != path)
			*ptr = 0;
	} else if (len > 0) {
		plen = strlen(path);
...
void do_detect_ftp(struct tcp_stream *a_tcp, struct supp **param_ptr)
{
	...
	for (;;) {
		...
		if (index2 - offset >= n_bytes)
			break;
		if (!strncasecmp(buf + index - offset, "cwd ", 4))	//比较两个字符串的前n个字节，n为第三个参数指定。
		{
			path_index = index + 4;
...
void detect_ftpd(struct tcp_stream *a_tcp, struct supp **param)
{
	if (a_tcp->nids_state == NIDS_JUST_EST) {
