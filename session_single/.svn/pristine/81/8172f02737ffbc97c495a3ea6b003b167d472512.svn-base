...
/**
 * @brief 处理smtp数据包，产生一个eml格式文件，发送信号给gmime解析函数
 *
 ...
 * @return  成功返回0，失败返回-1
 */
int do_smtp (const u_char *packet_content, struct tuple4 *key, unsigned int packlen, smtp_sbuffputdata sbuff_putdata, int sbuffer_id)
{   
    log_printf (ZLOG_LEVEL_DEBUG, " do_smtp(), %s__%d", __FILE__, __LINE__);
#if 0
    log_printf (ZLOG_LEVEL_DEBUG, "saddr==%lu ",key->saddr);
    log_printf (ZLOG_LEVEL_DEBUG, "daddr==%lu ",key->daddr);
    log_printf (ZLOG_LEVEL_DEBUG, "source==%u ",key->source);
    log_printf (ZLOG_LEVEL_DEBUG, "dest==%u ",key->dest);
#endif
    	
    zcsmtp_state_t  smtp_state;
    unsigned char   *packstr = NULL;
    hash_bucket     *pBucket = NULL;	
	SmtpSession_t	*p_session = NULL;
  SmtpSession_t   *p_sessionsrc = NULL;

    log_printf (ZLOG_LEVEL_DEBUG, "  ###############start##########################  ");	
	//log_printf (ZLOG_LEVEL_DEBUG, "key->source = %d ", key->source);
	//log_printf (ZLOG_LEVEL_DEBUG, "key->dest = %d ", key->dest);

#ifdef TCPIPPACK
    /* 如果是完整的数据包，先去掉tcp,ip层一些无用信息 */
	int len = packlen;
    packstr = (unsigned char *)(packet_content+34+len);
	packlen = strlen((char*)packstr);
#else
	/* 已经是smtp包了 */
    packstr = (unsigned char*)packet_content;
#endif

#if 0
	/* 只存储不解析,生成smtp数据流集合 */
    FILE *pfile = fopen ("smtp.tmp", "a+");
	fwrite (packstr, packlen, 1, pfile);
	if (pfile)
	{
	    fclose(pfile);
	    pfile = NULL;
	...
	/* 初次建hash桶 */
	log_printf (ZLOG_LEVEL_DEBUG, "Is this first build hash? ");
    log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
...
#ifdef TCPIPPACK
		/* 用于自测 */
	    log_printf (ZLOG_LEVEL_DEBUG, "*************first build hash*************** ");
        log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
		if((ftp_session_tbl = create_hash (100000, session_hash, (t_comp_func)session_comp))==NULL)
		{
	          log_printf (ZLOG_LEVEL_ERROR, "smtp create_hash fail !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ");
              log_printf (ZLOG_LEVEL_ERROR, "%s__%d ", __FILE__, __LINE__);
		}
#else
	    log_printf (ZLOG_LEVEL_ERROR, "fail, hash not created!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ");
        log_printf (ZLOG_LEVEL_ERROR, "%s__%d ", __FILE__, __LINE__);
		return -1;
#endif
    }
	...
	{
	    log_printf (ZLOG_LEVEL_DEBUG, "*************hash already build*************** ");
        log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
	}
    ...
    {
		  /* 不存在就创建 */
          log_printf (ZLOG_LEVEL_DEBUG, " not find key, add new key ");
          log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
	      LS_MEM_MALLOC (pBucket, hash_bucket);
		  LS_MEM_MALLOC (p_session, SmtpSession_t);
		  memset (p_session, 0, sizeof(SmtpSession_t));
		  p_session->next = NULL;
		  ...
		  /* 插入一个项目 */
		  log_printf (ZLOG_LEVEL_DEBUG, "insert to hash ");
          log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
          insert_hash (ftp_session_tbl, pBucket, sizeof(struct tuple4));
    }
	else
	{
		log_printf (ZLOG_LEVEL_DEBUG, "key have exist ");
        log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
	}

	/* 取出桶中项目 */
    log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
    p_sessionsrc = (SmtpSession_t*)pBucket->content;
	p_session = p_sessionsrc;

#if 0
    /* 每次都是在最新结点,处理相同的四员组,多次发送的情况 */
    while(p_session->next != NULL)
	{
        log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
		p_session = p_session->next;
	...
	if (packstr == NULL || packlen == 0)
	{
		 log_printf (ZLOG_LEVEL_WARN, "packet is null ");
	     log_printf (ZLOG_LEVEL_WARN, "%s__%d ", __FILE__, __LINE__);   
         return -1; 
	}

	/* 判断当前传输状态是否正在进行数据传输 */
	if (p_session->data_store_flag == DATA_BDAT_START)
	{
		/* 数据 */
		log_printf (ZLOG_LEVEL_DEBUG, "transmiting mail data");
        log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
        p_session->smtp_state = SMTP_STATE_READING_DATA;
	}
	/* 判断当前传输状态数据是否加密,加密不做处理，丢弃 */
	else if (p_session->smtp_state == SMTP_STATE_READING_STARTTLS)
	{
        log_printf (ZLOG_LEVEL_WARN, "transmiting STARTTLS, STARTTLS don`t do any thing ");
        log_printf (ZLOG_LEVEL_WARN, "%s__%d ", __FILE__, __LINE__);
        if (dispose_STARTTLS_cmd (p_sessionsrc, (char*)packstr, packlen) == -1)
		{
			return -1;
		}
	...
	{
		/* 命令或响应 */
		log_printf (ZLOG_LEVEL_DEBUG, "transmit request add response ");

        /* 根据包的头字符判断命令还是响应 */
        unsigned char head = packstr[0];
		log_printf (ZLOG_LEVEL_DEBUG, "head=%c ", head);
	    if ((head >='0') && (head <= '9'))
	    {
	       log_printf (ZLOG_LEVEL_DEBUG, "******************response********************** ");
           smtp_state =  SMTP_STATE_READING_RESPONSE; /* 响应 */
	    }
        else 
        {  
	       log_printf (ZLOG_LEVEL_DEBUG, "******************request********************** ");
           smtp_state = SMTP_STATE_READING_CMDS;     /* 命令 */
        }
	    //log_printf (ZLOG_LEVEL_DEBUG, "**************************************packstr=%s ", packstr);
        //log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);

		/* 设置状态 */
        p_session->smtp_state = smtp_state;
	}

    if (p_session->smtp_state ==  SMTP_STATE_READING_CMDS)     /* 处理命令 */
	{
		log_printf (ZLOG_LEVEL_DEBUG, "deal with request............. ");
        log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
        dispose_smtp_cmd (p_session, (char*)packstr, packlen);
	}
	else if (p_session->smtp_state ==  SMTP_STATE_READING_RESPONSE) /* 处理响应 */
	{
		log_printf (ZLOG_LEVEL_DEBUG, "deal with response............. ");
        log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
        dispose_smtp_response (p_session, (char*)packstr, packlen);
	}
	else if (p_session->smtp_state ==  SMTP_STATE_READING_DATA)     /* 处理DATA数据 */
	{	
		log_printf (ZLOG_LEVEL_DEBUG, "deal with DATA............. ");
        log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
        dispose_smtp_mail_body_data (p_session, (char*)packstr, packlen, sbuff_putdata, sbuffer_id);
	}
	else if (p_session->smtp_state ==  SMTP_STATE_READING_BDAT)     /* 处理BDAT数据 */
	{
		log_printf (ZLOG_LEVEL_DEBUG, "deal with BDAT............. ");
        log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
        dispose_smtp_mail_body_bdat (p_session, (char*)packstr, packlen, sbuff_putdata, sbuffer_id);
	}

#if 0
    if (p_session->session_quit_flag == TRUE)
	{

            /* 销毁哈希桶 */
	        log_printf (ZLOG_LEVEL_INFO, "free hash ");
	        remove_hash(ftp_session_tbl, pBucket, sizeof(struct tuple4));
	        /* 释放桶中资源 */
	        free_smtp(pBucket->content);
	        pBucket->content = NULL;
	        free(pBucket);
	        pBucket = NULL;
			log_printf (ZLOG_LEVEL_INFO, "p_session is NULL @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ ");
	        /* 将桶中的信息给返回变量 */
	        //cp_pbucket_to_session(&re_struc, p_session);
	}
#endif
	
    log_printf (ZLOG_LEVEL_DEBUG, "  ###############end##########################  ");	
    log_printf (ZLOG_LEVEL_DEBUG, "%s__%d ", __FILE__, __LINE__);
#if 0
	log_printf (ZLOG_LEVEL_INFO, "^^^^^^^^^^show-start^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ ");
    while_show_smtp(p_session);	
	log_printf (ZLOG_LEVEL_INFO, "^^^^^^^^^^^^show-end^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ ");
#endif
