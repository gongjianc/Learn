...
{
    int len, eoh; //eoh -- end of header?
    char **newspace;
    char *shead, *ebuf, *str;

    if (h->bufused < 2)        /*???????????? */ //why 2
        return EC_400;

    ebuf = h->buf + h->bufused - 2; //h->bufused 计算了/r/n 或许还有 \0
    ...
    *ebuf = '\0'; //
    ...
    h->headers[0] = h->buf;  //headers[0]存储完整的icap头? //有64个头的指针?
    ci_debug_printf(9, "\033[0;34m=====h->header[0] is \n %s\n===============\n\033[0m", h->buf);
    ...
    for (str = h->buf; str < ebuf; str++) {    /*Construct index of headers */
        ...
        {
            /* ci_debug_printf(9, "====================================00000==========\n"); */
            *str = ' ';
        }

        //处理完一行将headers的每一行存到h->headers[i]中 
        if (eoh) {
            *str = '\0';
            //h->size = 64, h->used表示已经分配的h->headers
            //可以扩容
            ...
            ci_debug_printf(9, "\033[0;34m=====headers[%d] is \n%s\n========\n\033[0m", h->used, str);
            h->used++; 
        }	  
    }     
    h->packed = 0;
