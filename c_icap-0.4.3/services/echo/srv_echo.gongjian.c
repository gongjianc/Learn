...
#define HTTP_FILE_HOST_MAX 8190
#define HTTP_FILE_ORIGIN_MAX 8190
#define HTTP_FILE_REFERER_MAX 8190
...
int echo_end_of_data_handler(ci_request_t * req)
{
    ...
    ci_debug_printf(5, "\033[0;34m\n");
    ci_debug_printf(5, "Content_length is %d\n", content_len);
    ci_debug_printf(5, "\033[0m \n");
    ...
    if (echo_read_from_net("<script type=\"text/javascript\">alert(\"hello world\");</script>",61, 1, req) == CI_ERROR){
        ci_debug_printf(9, "read from error\n");
    }
...
*/
    ...
    if ((httpinf->host = ci_headers_value(req->request_header, "Host")) != NULL) {
        ci_debug_printf(2, "DEBUG POST : %s\n", httpinf->post);
     }

     if((httpinf->origin = ci_headers_value(req->request_header, "Origin")) != NULL){
        ci_debug_printf(2, "DEBUG ORIGIN : %s\n", httpinf->origin);
     }

     if((httpinf->referer = ci_headers_value(req->request_header, "Referer")) != NULL){
        ci_debug_printf(2, "DEBUG REFERER : %s\n", httpinf->referer);
     }
