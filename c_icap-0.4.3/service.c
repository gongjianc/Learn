...
ci_service_module_t *add_service(ci_service_module_t *service)
{
    ...
    if (service_list == NULL) {
        //STEP = 32
        ci_debug_printf(9, "\033[0;37m==============service_list is NULL============\n[0m"); //by jayg
        service_list_size = STEP;
        service_list = malloc(service_list_size * sizeof(ci_service_module_t *));
        service_extra_data_list = malloc(service_list_size * sizeof(ci_service_xdata_t));
    }
    else if (services_num == service_list_size) {
        ci_debug_printf(9, "\033[0;37m==============service_list is-NOT-NULL============\n[0m"); //by jayg
        service_list_size += STEP;
        service_list = realloc(service_list, service_list_size * sizeof(ci_service_module_t *));
        service_extra_data_list = realloc(service_extra_data_list, service_list_size * sizeof(ci_service_xdata_t));
    }

    if (service_list == NULL || service_extra_data_list == NULL) {
        ci_debug_printf(1, "Fatal error: Cannot allocate memory! Exiting immediately!\n");
        exit(-1);
    }
