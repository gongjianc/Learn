...
static int ci_read_icap_header(ci_request_t * req, ci_headers_list_t * h, int timeout)
{
    ...
    ci_debug_printf(9, "\033[0;37m==========================BEFORE=====================================\n");
    ci_debug_printf(9, "h->buf is\n%s\n", h->buf);
    ci_debug_printf(9, "\033[0;37m=====================================================================\n");
    ci_debug_printf(9, "\033[0m");
    FILE *fp;
    if( (fp = fopen("/root/tmp/1.txt", "w+")) == NULL){
        perror("open file error");
    }
    char *temp = NULL;
    temp = (char *)malloc(ICAP_HEADER_READSIZE);
    memcpy(temp, buf_end + 2, ICAP_HEADER_READSIZE);
    fputs(temp, fp);
    fclose(fp);
    free(temp);
#if 0
    ci_debug_printf(9, "\033[0;37m===========================AFTER=====================================\n");
    ci_debug_printf(9, "h->buf is %s\n", h->buf);
    ci_debug_printf(9, "\033[0;37m=====================================================================\n");
    ci_debug_printf(9, "\033[0m");
#endif

    //h->bufused 指ICAP头的长度 只包含一个/r/n
    h->bufused = buf_end - h->buf;     /* -1 ; */

    //ci_debug_printf(9, "h->bufused is %d\n", h->bufused);
    
    //req->pstrblock_read 指向HTTP头，略过了ICAP头
    req->pstrblock_read = buf_end + 2; /*after the \r\n\r\n. We keep the first \r\n and the other dropped.... */
    //req->pstrblock_read_len 指向
...
static int read_encaps_header(ci_request_t * req, ci_headers_list_t * h, int size)
{
    ...
    while (remains > 0) {
        ...
        FILE *fp;
        if( (fp = fopen("/root/tmp/1.txt", "a+")) == NULL){
            perror("open file error");
        }
        char *temp = NULL;
        temp = (char *)malloc(bytes);
        bzero(temp, bytes);
        memcpy(temp, buf_end, bytes);
        fputs(temp, fp);
        fclose(fp);
        free(temp);
