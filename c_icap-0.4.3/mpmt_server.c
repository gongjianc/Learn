...
int thread_main(server_decl_t * srv)
{
    ...
    for (;;) {
        ...
        if (srv->current_req == NULL){
            ...
            ci_debug_printf(9, "\033[0;34m============================get new request===================\n\033[0m");
        }
        else{
            ret = recycle_request(srv->current_req, &con);
            ci_debug_printf(9, "\033[0;34m============================recycle request===================\n\033[0m");
        }
        ...
        keepalive_reqs = 0;
        do {
            if (MAX_KEEPALIVE_REQUESTS > 0 && keepalive_reqs >= MAX_KEEPALIVE_REQUESTS)
            ...
            if ((request_status = process_request(srv->current_req)) == CI_NO_STATUS) {
                ci_debug_printf(5, "Process request timeout or interrupted....\n");
    ...
    for (;;) {                 //Global for
        ...
        do {                  //Getting requests while we have free servers.....
            ...
            do {
                ...
                if (((conn.fd = accept(sockfd, (struct sockaddr *) &(conn.claddr.sockaddr), &claddrlen)) == -1)) {
...
void child_main(int sockfd, int pipefd)
{
    ...
    for (i = 0; i < CI_CONF.THREADS_PER_CHILD; i++) {
        ...
        ret = ci_thread_create(&thread, (void *(*)(void *)) thread_main,
                    (void *) threads_list[i]);
    ...
    ret = ci_thread_create(&thread, (void *(*)(void *)) test_ft, &thd_id);
    ...
    child_signals();// in order to make sure that all threads are up, by jayg
...
int start_server()
{
    ...
    if (ctl_socket < 0) {
        ci_debug_printf(1, "Error opening control socket %s: %s. Fatal error, exiting!\n", strerror(errno), CI_CONF.COMMANDS_SOCKET);
        exit(0);
    }

    if (!ci_proc_mutex_init(&accept_mutex, "accept")) {
        ci_debug_printf(1, "Can't init mutex for accepting conenctions. Fatal error, exiting!\n");
        exit(0);
    }
    childs_queue = malloc(sizeof(struct childs_queue));
    if (!create_childs_queue(childs_queue, 2 * CI_CONF.MAX_SERVERS)) {
    ...
    if (pid != 0) {
        ...
        ci_debug_printf(1, "Possibly a term signal received. Monitor process going to term all children\n");
        kill_all_childs();
        system_shutdown();
